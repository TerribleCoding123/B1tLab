...
void commandCheck(BuiltIns::UserInput &input) {
  ...
  if (command == "exit") {
    ...
    std::string pink{View::Utils::rgb666ToString(
        View::kReturnPink[0], View::kReturnPink[1], View::kReturnPink[2])};
    std::cout << pink
              << BinaryConversion::DefaultConversion::decToBin(input.tokens)
              << View::kReset << "\n\n";
    ...
    std::string pink{View::Utils::rgb666ToString(
        View::kReturnPink[0], View::kReturnPink[1], View::kReturnPink[2])};
    std::cout << pink
              << BinaryConversion::DefaultConversion::binToDec(input.tokens)
              << View::kReset << "\n\n";
    ...
    std::string output{BinaryConversion::StandardConversion::standardConverter(
        input.tokens, BinaryConversion::dec_to_bin)};
    ...
    std::string output{BinaryConversion::StandardConversion::standardConverter(
        input.tokens, BinaryConversion::bin_to_dec)};
...
int main() {
  ...
  while (input.flag != BuiltIns::f_exit) {
    ...
    if (input.unparsed_input.length() == 0)
      continue;
