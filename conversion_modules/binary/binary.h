...
namespace BinaryConversion {
	...
	//input on a frac format or a frac input on an int format will result in the loss of data warning
	//Note! There is an implicit format UNSIGNED, which is summoned only via the u or U prefix after the number
	const inline std::vector<std::string_view> int_formats{"UNSIGNED","SIGN_MAG", "ONES_COMP", "TWOS_COMP" };
	...
	namespace Utils {
		...
		//Converts the absolute value of fractional that is less than one (e.g. 1/3, 1/5, 5/5) to a mathematically accurate binary sequence
	...
	namespace DefaultConversion{
		...
		(e.g. 0.1 dec->bin = 0.000(1100) ), if there is 
		an irrational number it will be written until the 64-bit limit is reached, and then noted with three dots at the end
	...
	namespace StandardConversion {
		...
		*/
		...
		Value-based works like in any ordinary converter where you pass value and opional resize flag as an argument
		Format-based has 5 sub-types: 
		--set_global sets choosen format for both SDEC_TO_BIN and SBIN_TO_DEC 
		--set_local sets choosen format explicitly for given converter (e.g. SDEC_TO_BIN --set_local FORMAT will only affect SDEC_TO_BIN)
		--set_default resets formats for the whole STANDARD_CONVERSION family to default ones (TWOS_COMP, IEEE754_BINARY)
		--show_global shows all of the currently active formats for the STANDARD_CONVERISON family 
		--show_local shows all of the currently active formats for the specific converter
		...
		*/
		...
		const inline std::vector<std::string_view> flag_mode{ "--set_global", "--set_local", "--set_default", "--show_global", "--show_local"};
		
		//By checking the spelling of the input, returns two modes: Default (Value, flag), Format Configurable 
